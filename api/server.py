...
fields_predicates = info.model('Predicates', {
    'json': fields.List(fields.List(fields.String), example=[
        ['Q30', 'Q60'], ['Q166262', 'Q25191']
    ])
})

fields_objects = info.model('Objects', {
    'json': fields.List(fields.String, example=[
        'Q30', 'Q166262'
    ])
})

fields_sameas = info.model('SameAS', {
    'json': fields.List(fields.String, example=[
        'Q30', 'Q31'
    ])
})

fields_literals = info.model('Literals', {
    'json': fields.List(fields.String, example=[
        'Q30', 'Q31'
    ])
})

fields_types = info.model('Concepts', {
    'json': fields.List(fields.String, example=[
        'Q30', 'Q31'
    ])
})

fields_literal_recognizer = info.model('LiteralRecognizer', {
    'json': fields.List(fields.String, example=[
        "50",
        "12/11/1997",
        "https://www.unimib.it/",
...
fields_labels = info.model('Labels', {
    'json': fields.List(fields.String, example=[
        'Q30', 'Q31'
    ])
})

fields_rdf2vec = info.model('RDF2Vec', {
    'json': fields.List(fields.String, example=[
        'Q30', 'Q31'
    ])
})

fields_column_analysis = info.model('ColumnAnalysis', {
    'json': fields.List(fields.List(fields.String),
                        example=[['10', '100', '1000'], ['12/11/1997', '26/08/1997', '14/05/2016'],
                                 ['London', 'New York', 'Paris']])
})

fields_ner = info.model('NERRecognizer', {
    'json': fields.List(fields.String, example=[
        'Albert Einstein was a German Scientist', 'Alan Turing was an English Mathematician'
    ])
})

fields_cells = api.model("Cells",  {
...
@api.doc(
    responses={200: "OK"},
    description='Infos about this endpoint'
)
class Info(Resource):
    def get(self):
        info_obj = {
...
@lookup.route("/entity-retrieval-with-array")
@api.doc(
    responses={200: "OK", 404: "Not found",
               400: "Bad request", 403: "Invalid token"},
    params={
        "limit": "The number of entities to be retrieved. The default value is 100.",
        "token": "Private token to access the API.",
        "kg": "The Knowledge Graph to query. Available values: <code>dbpedia</code> or <code>wikidata</code>. Default is <code>wikidata</code>.",
        "fuzzy": "Set this param to True if fuzzy search must be applied. Default is <code>False</code>.",
        "ngrams": "Set this param to True if ngrams search must be applied. Default is <code>False</code>.",
        "types": "Types to be matched in the Knowledge Graph as constraint in the retrieval. Add Types separeted by spaces. E.g. Scientist Philosopher Person"
    },
    description="Given a string as input, the endpoint performs a search in the specified Knowledge Graph."
)
class LookupWithArray(Resource):
    @entity.doc(body=fields_cells)
    def post(self):
        """
           Perfom more query using array
        """
        parser = reqparse.RequestParser()
        parser.add_argument('limit', type=str, location="args")
        parser.add_argument('token', type=str, location="args")
        parser.add_argument('kg', type=str, location="args")
        parser.add_argument('fuzzy', type=str, location="args")
        parser.add_argument('ngrams', type=str, location="args")
        parser.add_argument('types', type=str, location="args")
        args = parser.parse_args()
        ...
        try:
            cells = request.get_json()["cells"]
        except:
            return {"Error": "Invalid Json"}, 400

        is_fuzzy_valid, fuzzy_value = params_validator.validate_bool(fuzzy)
        is_ngrams_valid, ngrams_value = params_validator.validate_bool(ngrams)

        if not is_fuzzy_valid:
            return fuzzy_value

        if not is_ngrams_valid:
            return ngrams_value

        kg_is_valid, kg_error_or_value = params_validator.validate_kg_lookup(database, kg)
        if not kg_is_valid:
            return kg_error_or_value
        ...
        try:
            results = lookup_retriever.search(cells, limit=limit_error_or_value, kg=kg_error_or_value, 
                                                fuzzy=fuzzy_value, ngrams=ngrams_value, types=types)
        except Exception as e:
            return build_error(f"Elastic error: {str(e)}", 400, traceback=traceback.format_exc())

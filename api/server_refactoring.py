...
class LookupRetriever:
    ...
    def _get_final_candidates_list(
        self, result, name, kg, ambiguity_mention, corrects_tokens, ntoken_mention, length_mention, input_ner_type=None
    ):
        ...
        for entity in result:
            ...
            #################################################################################################
            # Skip entities that don't match the NERtype, if input_ner_type is provided
            if input_ner_type and input_ner_type not in entity.get("NERtype", []):
                continue
            #################################################################################################
    ...
    def create_query(self, name, fuzzy=False, types=None, kind=None, NERtype=None, language=None):
        ...
        if NERtype:
            query_base["query"]["bool"]["filter"].append({"terms": {"NERtype": [NERtype]}})    # terms is used instead of term, as terms matches any of the values in a list.
        #########################################################################################################
